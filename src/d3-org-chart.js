...
export class OrgChart {
    ...
    calculateCompactFlexPositions(root) {
        ...
        root.eachBefore(node => {
            if (node.children) {
                const compactChildren = node.children.filter(d => d.flexCompactDim && d.data.positionType == nodeType.REGULAR);
    ...
    zoomed(event, d) {
        if (event.sourceEvent && event.sourceEvent.altKey == false && event.sourceEvent.type == "wheel") return;
        const attrs = this.getChartState();
